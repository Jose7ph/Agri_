...
class BlockModel(app: Application) : AndroidViewModel(app),
    IWorkPlan by WorkPlanImpl(),
    IWorkPage by WorkPageImpl(Block.TYPE_BLOCK),
    IWorkEdit by WorkEditImpl(),
    IWorkBreakpoint by WorkBreakpointImpl() {

    val clearFlag = MutableStateFlow(false)
    ...
    fun setBP(blockPlan: BlockPlan, clearHighlightedBlocks: Boolean = true) {
        exeTask {
            setBlockPlan(blockPlan)
            highlightBlocks.emit(listOf(blockPlan))
            if (clearHighlightedBlocks) {
    ...
    //map 点击地块设置高亮地块
    fun setHighlightBlocks(uniqueIds: List<String>) {
        ...
        setHighlightBlocksByUniqueIds(uniqueIds = uniqueIds)
        if (highlightedBlocks.size == 1) {
            val currentBP = highlightedBlocks[0]
            if (currentBP.uniqueId() == selectedBP?.uniqueId()) {
                return
            }
            setBP(highlightedBlocks[0])
        }
    }
